...
local function fireForPlayers(remote: any, players: any, ...)
	print("Firing create remote fireForPlayers")
...
function ServerReplica.new(params: ReplicaParams)
	...
	if replicateAll then
		...
		print("Firing create remote")
		-- Replicate replica to provided players
		fireForPlayers(CreationRemote, replication, params)
	end
	...
	self._trove:Add(Timer.Simple(UPDATE_RATE, function()
		...
		if #self.ChildQueue > 0 then
			print("Sending update to queue")
			print(self)
			if replicateAll then
				ChildRemote:FireAllClients(self.ReplicaId, self.ChildQueue)
			else
				fireForPlayers(ChildRemote, replication, self.ReplicaId, self.ChildQueue)
			end
